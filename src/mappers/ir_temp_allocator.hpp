...
namespace splicpp
{
	...
	{
		const std::map<ir_temp, ir_temp> tmap;
		...
		ir_temp_allocator(const std::map<ir_temp, ir_temp>& tmap)
		: tmap(tmap)
		, stmts()
		, acc()
		{}
	...
	public:
		static std::vector<s_ptr<const ir_stmt>> apply(const std::vector<s_ptr<const ir_stmt>>& stmts, const std::map<ir_temp, ir_temp>& reserved_temps, const std::vector<ir_temp>& scratch_temps);
